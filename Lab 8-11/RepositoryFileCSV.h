...
template<class T> RepositoryFileCSV<T>::RepositoryFileCSV(const char* file, Serializer<T>*sr) : RepositoryFile<T>() {
	RepositoryFile<T>::fileName = "";
	s = sr;
	loadFromFile(file);
}

template<class T> void RepositoryFileCSV<T>::loadFromFile(const char* file) {
...
template<class T>int RepositoryFileCSV<T>::addElem(T ent) {
	int rez = RepositoryFile<T>::addElem(s->goodObj(ent));
	saveToFile();
	return rez;
}

template<class T>int RepositoryFileCSV<T>::delElem(T ent) {
...
template<class T> void RepositoryFileCSV<T>::updateElem(T ent, T newEnt) {
	typename list<T>::iterator it;
	for (it = RepositoryFile<T>::elem.begin(); it != RepositoryFile<T>::elem.end(); ++it){
		if (s->isEqual((*it), ent) == true){
...
template<class T>bool RepositoryFileCSV<T>::findElem(T ent) {
	typename list<T>::iterator it;
	for (it = RepositoryFile<T>::elem.begin(); it != RepositoryFile<T>::elem.end(); it++) {
		if (s->isEqual(*it, ent) == true)
			return true;
